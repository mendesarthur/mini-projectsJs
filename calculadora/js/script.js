document.addEventListener("DOMContentLoaded", function () {
    ...
    let btnLimparTela = document.getElementById("limparTela");
    let btnApagarAnterior = document.getElementById("apagarAnterior");
    ...
    function verificarResulatado() {
        try {
            let aux = tela.value.substring(tela.value.length - 1, tela.value.length);
            if (verificarOperador(aux)) {
                apagarAnterior();
            }

            let valorCalculado = eval(tela.value); //calcular o conteúdo da string
